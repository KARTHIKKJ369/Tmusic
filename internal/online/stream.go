...
// and concurrently downloads the album artwork.
func ResolveAndCache(ctx context.Context, track OnlineTrack, onStatus func(string)) (string, []byte, error) {
	...
	// Fast extraction: If track is from YouTube with a valid video ID, pass the direct URL!
	...
	if track.Source == "youtube" && len(track.ID) >= 8 && !strings.HasPrefix(track.ID, "itunes_") {
		// Direct YouTube video ID: skips entire search phase for ultra-fast startup!
	...
	if err != nil {
		// Retry without "Official Audio" if it was a search
